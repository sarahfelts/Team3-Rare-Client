...
import { createGame, getGameTypes, updateGame, getGame } from '../api/gameData';
...
const GameForm = ({ user, game = null, id = '' }) => {
  ...
  useEffect(() => {
    if (id) {
      getGameDetails(id).then((gameDetails) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (id) {
      updateGame(id, gameToSave).then((updatedGameId) => router.push(`/game/${updatedGameId}`));
    } else {
